...
class OverPaymentsRepo:
    ...
    def getOverpayments(db: Session, user: UserModel, skip: int, limit: int):
        ...
        try:
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error in getting all over payments: {e}")
    ...
    def getUserOverpayments(db: Session, user: UserModel, phoneNumber: str):
        ...
        try:
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error in getting user over payments: {e}")
